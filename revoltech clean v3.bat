...
set "hashedPass=!password!"

:: Perform basic password hashing (insecure)
set "hashedPass=%hashedPass:a=1%"
set "hashedPass=%hashedPass:b=2%"
set "hashedPass=%hashedPass:c=3%"

:: Perform basic username check
if /i "%username%"=="naingko" (
    call :authenticate
) else (
...
::echo (1) -----only clean all temp, recent, recycle bin
::echo (2) -----all clean step by step before hand over to customer
::set /p user_input= choose 1 or 2 =
::if /i %user_input%==1 goto only
::if /i %user_input%==2 (goto step by step) else (goto invalid)
...
Echo.                    ^|   [5]   only clean all temp, recent, recycle bin        ^|
Echo.                    ^|                                                         ^|
Echo.                    ^|  [6]all clean step by step before hand over to customer ^|
Echo.                    ^|                                                         ^|
Echo.                    ^|   [7] Exit                                              ^|
Echo.                    ^|_________________________________________________________^|                      
ECHO.

choice /C:123456789 /N /M ".                   Enter Your Choice [1,2,3,4,5,6,7] : "
if errorlevel 7 goto:Exit
if errorlevel 6 goto:Step by step
if errorlevel 5 goto:only
...
start explorer.exe
echo -------------succeffuly deleted temp/%temp%/prefetch/recent/recycle bin---------
echo ***********cleanmgr mr file delete pr*********
start cleanmgr
echo *******run box recent shinn pr ml*********
pause
REG Delete HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\RunMRU /VA /F
REG Delete HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\TypedPaths /VA /F 

set /p user_input= service phwint mr lrr(y/n)?
...
:invalid
color 4
echo ********%user_input% is a invalid entry. try again!********
pause
goto MIANMENU
